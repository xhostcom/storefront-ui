...
export const SfSticky = ({ children }) => {
  ...
  useEffect(() => {
    if (isSticky) {
      if (ref.current.nextSibling) {
        ref.current.parentElement.style.paddingTop = `${stickyValues.height +
          stickyValues.padding.top}px`;
      }
    } else {
  ...
  const scrollEnd = () => {
    return (
      parent.parentHeight + parent.parentTop - stickyValues.height - stickyValues.padding.bottom
    );
  }
  ...
  const toggleSticky = () => {
    if (
      scrollY >= parent.parentTop + stickyValues.top &&
      scrollY < scrollEnd()
    ) {
      setIsSticky(true)
    } else {
