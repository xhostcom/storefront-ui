...
export const SfPagination = ({ current, total, visible, onClick, iconPrev, iconNext }) => {
  ...
  const setLimitedPageNumber = () => {
    ...
    return listOfPageNumbers().slice(current - Math.ceil(visible / 2), current + Math.floor(visible / 2));
  };
    ...
    <nav className="sf-pagination">
      <ul className="sf-pagination__list">
        ...
        {showFirst && (
          <>
            <li className="sf-pagination__item">
              <button onClick={() => go(1)} className="sf-pagination__button">
                1
              </button>
            </li>
            <li className="sf-pagination__item">...</li>
          </>
        )}
        {limitedPageNumbers().map((number) => (
          <li key={number} className="sf-pagination__item">
            <button
              onClick={() => go(number)}
