...
export const SfSelect = ({ classname, label, options, selected, errorMessage, size, valid, required, onSelect }) => {
  ...
  const onKeyPressed = (ev) => {
    if (ev.key === "ArrowUp" || ev.keyCode === 38) {
      if (!selectedValue) {
        ...
        setSelectedValue(options.findIndex(el => el.value === selectedValue.value) === 0
        ? options[0]
      ...
      if (!selectedValue) {
        ...
        setSelectedValue(options.findIndex(el => el.value === selectedValue.value) === options.length - 1
  ...
  return (
    <div
      ...
      {valid !== undefined && <div
        className="sf-select__error-message">
